KISSY.add("app/views/manage/media/media_imgManage", function (S, View, MM, VOM, Node, Util, Uploader, Calendar, FixedHead) {
    var $ = Node.all;
    ...
    return View.extend({
        ...
        components: function () {
            ...
            S.use(plugins, function(S,Auth) {
                ...
                var uploader = new Uploader('#J_UploaderBtn', {
                    ...
                    action: 'http://sitemanager.jae.taobao.com/sitemanager/imageservice/upload.htm',
                    multiple: true
                }).plug(new Auth({
                    allowExts:allowExts,
                ...
                $('.ks-uploader-button').one('.btn-text').html('新增图片');
        ...
        'show<mouseover>': function (e) {
            ...
            var curNode = $('#' + e.currentId);
            var clone;
            if(curNode.siblings('img').length == 0){
                var src = curNode.attr('src').replace(/_80x80\.jpg$/, '');
                clone = S.one('<img src="' + src +'" class="clone" width="180">');
                clone.insertAfter(curNode);
            }
            curNode.siblings('.clone').show();
        },
        'hide<mouseout>': function (e) {
            var curNode = $('#' + e.currentId);
            var preview = curNode.siblings('.clone');
